...
class LinkedList:
    ...
    def partition(self, start, end):
        """Función para particionar la lista y devolver el pivote"""
    ...
    def quicksort_recursive(self, start, end):
        """Ordenación rápida recursiva en la lista enlazada"""
        if start is None or end is None or start == end:
            return

        pivot_prev = self.partition(start, end)
        
        # Ordenar la lista antes del pivote
        self.quicksort_recursive(start, pivot_prev)

        # Ordenar la lista después del pivote
    ...
    def sort_by_quick(self):
        """Ordenación rápida en la lista enlazada completa"""
        end = self.head
...
class LinkedListApp:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("Lista Enlazada - Ordenación Rápida (Quicksort)")
        ...
        self.sort_button = tk.Button(root, text="Ordenar Lista por Quicksort", command=self.sort_list)
    ...
    def sort_list(self):
        """Ordenar la lista por Quicksort y mostrar el resultado con el tiempo de ejecución"""
        ...
        self.result_text.delete(1.0, tk.END)
        self.result_text.insert(tk.END, f"Lista ordenada por Quicksort:\n{self.linked_list.display()}\n")
