...
class LinkedList:
    ...
    def get_node_at_index(self, index):
        """Obtener el nodo en un índice específico"""
        current = self.head
    ...
    def sort_by_shell(self):
        """Ordenación Shell en la lista enlazada"""
        n = self.get_length()
        gap = n // 2
        while gap > 0:
            for i in range(gap, n):
                ...
                j = i
                # Realizar un desplazamiento de nodos separados por 'gap'
...
class LinkedListApp:
    def __init__(self, root):
        ...
        self.root = root
        self.root.title("Lista Enlazada - Ordenación Shell")
        ...
        self.sort_button = tk.Button(root, text="Ordenar Lista por Shell", command=self.sort_list)
    ...
    def sort_list(self):
        """Ordenar la lista por Shell y mostrar el resultado con el tiempo de ejecución"""
        ...
        self.result_text.delete(1.0, tk.END)
        self.result_text.insert(tk.END, f"Lista ordenada por Shell:\n{self.linked_list.display()}\n")
